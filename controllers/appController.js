...
exports.getTaskListV1 = async (req, res) => {
  ...
  try {
    let query = `
      ...
      LEFT JOIN contact_types ct ON ct.id = (SELECT contact_type_id FROM casefile_interactions WHERE casefile_id = cf.cfid ORDER BY date_created DESC LIMIT 1)
      LEFT JOIN contact_statuses cs ON cs.id = (SELECT contact_status_id FROM casefile_interactions WHERE casefile_id = cf.cfid ORDER BY date_created DESC LIMIT 1)
